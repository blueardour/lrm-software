...
char * getFileType(char * FilePath)
{
	...
	{
		if(FilePath[i] == '.' ) type = FilePath + i + 1;
	}
	return type;
}
...
int generate_queries(struct Options * op)
{
  FILE * database, * fp;
  unsigned long i,j,k;
  unsigned long dlen, qlen, postion;
  unsigned long count;
  char *string, *buffer;
  char c;
	...
	{
		printf("'ex' file is needed for the reference\r\n");
		return -3;
	}
  ...
  {
    srand(stime());
    postion = rand()%(dlen-op->length-op->length/100);
    if(fseek(database,postion,SEEK_SET) == -1)
    {
      printf("Fseek Error\r\n");
      return -4;
    }
    ...
    fprintf(fp,"> name(%s)-index[%ld]-position:%ld.(err:%d, length:%ld)\r\n%s\r\n",\
						getFileName(op->database),i,postion,op->error,count,buffer);
  }
