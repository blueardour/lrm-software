//  ******************************************************************//
//  author: chenp
//  description: build cache of fingerprint database
//  version: 1.0
//	date: 2014-05-22
//  ******************************************************************//
...
int cache_fingerprint(struct Sort_Options * op)
{
	char c;
	FILE * fp, * database;
	Fingerprint pt;
	...
	fprintf(stdout, "===> Cache uspt file...%d\r\n", op->verbose & 0x04);
	if(op->verbose & 0x04)
	{
		fp = fopen(op->uspt, "rb");
  		if(fp == NULL)
		...
		for(i=0; i<header.items; i++)
		{
			...
			cache_insert(getKey(pt, FPSize), pt.pos, header.length);;
